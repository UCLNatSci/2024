...
```{math}
\vec{r}_{cm} = \dfrac{\sum_{j=1}^N m_j\,\vec{r}_j}{\sum_{j=1}^N m_j}.
```
...
```{math}
\vec{p} = \sum_j \vec{p}_j = \sum_j m_j\,\vec{v}_j.
```

By definition, $\vec{v}_j = \dot{\vec{r}}_j$. Hence:

```{math}
\vec{p} = \sum_j m_j\,\dot{\vec{r}}_j = M\,\dot{\vec{r}}_{cm},
```
...
```{math}
m_i\,\dot{\vec{v}}_i = \dot{\vec{p}}_i = \vec{F}_i + \sum_{j=1}^N \vec{F}_{ij}.
```
...
```{math}
\sum_{i=1}^N \dot{\vec{p}}_i = \sum_{i=1}^N \vec{F}_i + \sum_{i=1}^N \sum_{j=1}^N \vec{F}_{ij}.
```
...
```{math}
\sum_{i=1}^N \dot{\vec{p}}_i = \dot{\vec{p}} = \sum_{i=1}^N \vec{F}_i,
```
...
```{math}
\frac{m}{3} (\vec{v_1} + \vec{v_2} + \vec{v_3}) = m\vec{v_0}
```

Since we are given $\vec{v_1} = \frac{\vec{v_0}}{2}$, this becomes:

```{math}
\vec{v_2} + \vec{v_3} = 3 \vec{v_0} - \frac{\vec{v_0}}{2} = \frac{5}{2} \vec{v_0}
```
...
```{math}
\frac{d\vec{L}}{dt} = \sum_i m_i\,\dot{\vec{r}}_i \times \vec{v}_i + \sum_i m_i\,\vec{r}_i \times \dot{\vec{v}}_i.
```
...
```{math}
\frac{d\vec{L}}{dt} = \sum_i \vec{r}_i \times (\vec{F}_i + \sum_j \vec{F}_{ij}).
```
...
```{math}
\vec{r}_i = \vec{r}_{cm} + \overline{\vec{r}}_i.
```
...
```{math}
\vec{v}_i = \vec{v}_{cm} + \overline{\vec{v}}_i.
```

Using this notation, the total angular momentum becomes:

```{math}
\vec{L} = \sum_{i=1}^N m_i\,(\vec{r}_{cm} + \overline{\vec{r}}_i) \times (\vec{v}_{cm} + \overline{\vec{v}}_i)
```
```{math}
= \sum_{i=1}^N m_i\,(\vec{r}_{cm} \times \vec{v}_{cm}) + \sum_{i=1}^N m_i\,(\vec{r}_{cm} \times \overline{\vec{v}}_i) + \sum_{i=1}^N m_i\,(\overline{\vec{r}}_{i} \times {\vec{v}}_{cm}) + \sum_{i=1}^N m_i\,(\overline{\vec{r}}_{i} \times \overline{\vec{v}}_i).
```
...
```{math}
T = \sum_{i=1}^N \frac{1}{2} m_i v_i^2 = \sum_{i=1}^N \frac{1}{2} m_i \vec{v}_i \cdot \vec{v}_i.
```
...
![A system of two bodies with masses $m_1$ and $m_2$, with respective positions $\overline{\vec{r}}_1$ and $\overline{\vec{r}}_2$ relative to the centre of mass.](Figs/twobody.png)
...
```{math}
  \dfrac{m_1}{m_1+m_2}\,\overline{\vec{r}}_1 + \dfrac{m_2}{m_1+m_2}\,\overline{\vec{r}}_2 = \vec{0} \quad \Rightarrow \quad m_1\,\overline{\vec{r}}_1 = - m_2 \, \overline{\vec{r}}_2.
```
...
```{math}
    m_1\,\dfrac{d^2\overline{\vec{r}}_1}{dt^2} = \vec{F}_{12} = -\dfrac{G m_1 m_2}{R^3} \vec{R}, \quad
    m_2\,\dfrac{d^2\overline{\vec{r}}_2}{dt^2} = \vec{F}_{21} =  \dfrac{G m_1 m_2}{R^3} \vec{R}
```
...
```{math}
    \ddot{\vec{R}} = -\dfrac{G m_1 m_2}{R^3}\vec{R} \left(\dfrac{1}{m_1} + \dfrac{1}{m_2}\right).
```
...
```{math}
    \mu\,\ddot{\vec{R}} = -\dfrac{G m_1 m_2}{R^2}\vec{e}_R,
```
...
![Artist's impression of Cygnus X-1](Figs/Cygnus_X-1.png)  
*Credit: ESA/HST*
...
```{math}
\Delta({m\vec{v}}) =\int_{t_1}^{t_1 + \Delta t} \vec{F}\,dt.
```
...
```{math}
\vec{p}_1 \cdot \vec{p}_1 = (\vec{p}'_1 + \vec{p}'_2)\cdot(\vec{p}'_1 + \vec{p}'_2)
= {{p}'_1}^2 + {{p}'_2}^2 + 2 \vec{p}'_1 \cdot \vec{p}'_2.
```
...
```{math}
\overline{\vec{p}}_1 + \overline{\vec{p}}_2 = 0 = \overline{\vec{p}}_1' + \overline{\vec{p}}_2',
```
...
```{math}
\overline{\vec{v}}'_1 = \vec{v}'_1 - \vec{v}_{cm} = \vec{v}'_1 - \dfrac{m_1 \vec{v}_1}{m_1 + m_2}
```
...
![Coordinates for calculating the centre of mass of a solid hemisphere.](Figs/hemisphere.png)
...
$ \vec{r}_i \times m_i \vec{v}_i $. The $ z $-component of this cross product is
```{math}
...
```{math}
N_z = \dfrac{dL_z}{dt} = \dfrac{d(I_z \omega)}{dt}.
```
...
![Coordinates for calculating the moment of inertia of a rod: (a) about one end and (b) about the centre of the rod.](Figs/rodmoi.png)
...
```{math}
    ...
    \frac{d}{dt}(\delta y) \, dt.
```
Integrating the final expression by parts gives:
```{math}
   \int_{t_1}^{t_2} m \dot{y}\,
    \frac{d}{dt}(\delta y) \, dt = \left[ m \dot{y} \, \delta y\right]_{t_1}^{t_2} -
...
```{math}
    \dfrac{\partial L}{\partial r} = \dfrac{d}{dt} \left( \dfrac{\partial L}{\partial \dot{r}}  \right)     \rightarrow 
...
```{math}
    \dfrac{\partial L}{\partial \theta} = \dfrac{d}{dt} \left( \dfrac{\partial L}{\partial \dot{\theta}}  \right)     \rightarrow 
    0 = \dfrac{d}{dt} \left( m r^2 \dot{\theta} \right).
```
