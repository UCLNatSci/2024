...
### Investigating the behaviour of $ G(x, t) $ near the point $ x = t $
...
```{math}
I_0(t, \epsilon) = \int_{t - \epsilon}^{t + \epsilon} a_0(x)\, G(x, t)\,\mathrm{d}x 
```
...
```{math}
I_2(t, \epsilon) &= \int_{t - \epsilon}^{t + \epsilon} a_2(x) G''(x, t)\mathrm{d}x = \left[ a_2(x) G'(x, t) \right]_{t - \epsilon}^{t + \epsilon}- \int_{t - \epsilon}^{t + \epsilon} a_2'(x)\, G'(x, t)\,\mathrm{d}x 
\\&= \left[ a_2(x) G'(x, t) - a_2'(x) G(x, t)\right]_{t - \epsilon}^{t + \epsilon} + \int_{t - \epsilon}^{t + \epsilon} a_2''(x)\, G(x, t)\,\mathrm{d}x
 ```
...
### Defining the linear combinations of coefficients
...
```{math}
J_0 = \lim_{\epsilon \to 0} \left[\int_{t - \epsilon}^{t + \epsilon} p_0(x) G(x, t)\mathrm{d}x \right] = p_0(t)\lim_{\epsilon \to 0}\left[ \int_{t - \epsilon}^{t + \epsilon} G(x, t)\mathrm{d}x\right],
```
...
then the continuity of $ G(x, t) $ and $ \int G(x, t) \,\mathrm{d}x $ holds (i.e., the Green's function is smooth and continuous), which leads to $ J_2 = 0 $, $ J_1 = 0 $, and $ J_0 = 0 $, contradicting the equation $ J_0 + J_1 + J_2 = 1 $.

Alternatively, if we assume that $ G'(x, t) $ has a finite discontinuity at $ x = t $, which means we can have:

```{math}
\lim_{\epsilon \to 0} \left( G'(t + \epsilon, t) - G'(t - \epsilon, t) \right) = 1,
```

BUT keep $ G(x, t) $ and $ \int G(x, t) \,\mathrm{d}x $ as continuous (i.e., the function is continuous but not smooth), i.e., $ J_2 = 1 $, $ J_1 = J_0 = 0 $, so consequently:
...
```{math}
p_2(t)\lim_{\epsilon \to 0} \left( G'(t + \epsilon, t) - G'(t - \epsilon, t) \right) = 1,
```
...
and write down solutions for the homogeneous equation for $ 0 \leq x < t $ and $ t < x \leq \frac{\pi}{2} $ (note that these solutions should be different because of the discontinuity at $ x = t $):

```{math}
\begin{cases}
G(x, t) = A(t) \sin(x) + B(t) \cos(x), & \text{for }  0 \leq x < t, \\
G(x, t) = C(t) \sin(x) + D(t) \cos(x), & \text{for }  t < x \leq \pi/2.
\end{cases}
```
...
From the boundary conditions, we have for $ 0 \leq x < t$:
...
\begin{split}G(x, t) =
\begin{cases}
A(t) \sin(x), & \text{for } 0 \leq x < t, \\
D(t) \cos(x), & \text{for } t < x \leq \pi/2.
\end{cases}\end{split}
```
...
\begin{split}G(x, t) =
\begin{cases}
- \cos(t) \sin(x), & \text{for } 0 \leq x < t, \\
- \sin(t) \cos(x), & \text{for } t < x \leq \pi/2.
\end{cases}\end{split}
```

### Finding the solution using the Green’s function
...
Notice that we are switching from a $ x $ space to a $ t $ space, so the specific parts of the Green's function in the integrand switch around, hence we have $ G_{x > t}(x, t) $ in the first part, $ G_{x < t }(x, t) $ in the second.

For $x > t$:

```{math}
\int_0^x G_{x > t}(x, t) f(t) \,\mathrm{d}t = \int_0^x (-\sin(t) \cos(x)) (t) \,\mathrm{d}t = -\cos(x)\left[-t \cos(t) + \sin(t)\right]_0^x
```

hence

```{math}
\int_0^x G_{x > t}(x, t) f(t) \,\mathrm{d}t = \cos(x)\left(x\cos(x)- \sin(x)\right)
```

For $ x < t$:

```{math}
\int_x^{\pi/2} G_{x < t}(x, t) f(t) \,\mathrm{d}t = \int_x^{\pi/2} (-\cos(t) \sin(x)) (t) \,\mathrm{d}t = -\sin(x)\left[t\sin(t)+\cos(t) \right]_x^{\pi/2}
```

hence

```{math}
\int_x^{\pi/2} G_{x < t}(x, t) f(t) \,\mathrm{d}t = \sin(x) \left(-\frac{\pi}{2} + x\sin(x)+\cos(x)\right)
```

Thus, a particular solution is:

```{math}
y(x) = x\left(\sin^2(x)+\cos^2(x)\right)-\cos(x)\sin(x) + \sin(x)\cos(x) - \frac{\pi}{2} \sin(x) = x - \frac{\pi}{2} \sin(x).
```
